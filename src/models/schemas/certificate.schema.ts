...
export const createCertificateSchema = z.object({
  directoryPath: z.string()
    ...
    .refine(path => !path.includes('~'), 'Path cannot contain ~ (home directory expansion not allowed)')
    .refine(path => !path.includes('\0'), 'Path cannot contain null bytes')
    .optional(), // Optional - will use default from config

  permissions: z.array(z.enum(['sftp', 'ssh', 'read-only', 'read-write', 'read-write-modify']))
  ...
  responseType: z.enum(['md', 'cert'])
    .optional(), // Optional - default is "md"

  authenticatorToken: z.string()
...
}).refine(data => data.ttl !== undefined || data.permissions !== undefined || data.status !== undefined, {
